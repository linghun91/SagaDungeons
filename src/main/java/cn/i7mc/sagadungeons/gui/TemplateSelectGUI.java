...
public class TemplateSelectGUI extends AbstractGUI {
    ...
    public TemplateSelectGUI(SagaDungeons plugin, Player player) {
        super(plugin, player, "&6选择副本模板", 54);
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot >= 10 && slot < 45 && slot % 9 != 0 && slot % 9 != 8) {
            ...
            // 发送消息
            if (success) {
                MessageUtil.sendMessage(player, "dungeon.create.success", 
                        MessageUtil.createPlaceholders("id", templateName));
            } else {
                MessageUtil.sendMessage(player, "dungeon.create.fail");
            }
        }
    }
    ...
    private ItemStack createTemplateItem(DungeonTemplate template) {
        ...
        if (template.hasMoneyCost()) {
            lore.add("&7需要金币: &e" + template.getMoneyCost());
        }
        
        if (template.hasPointsCost()) {
            lore.add("&7需要点券: &e" + template.getPointsCost());
        }
        
        if (template.hasLevelRequirement()) {
            lore.add("&7需要等级: &e" + template.getLevelRequirement());
        }
        
        lore.add("");
        lore.add("&7超时时间: &e" + template.getDefaultTimeout() / 60 + " 分钟");
