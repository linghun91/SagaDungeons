...
public class ItemConditionManageGUI extends AbstractGUI {
    ...
    public ItemConditionManageGUI(SagaDungeons plugin, Player player, String templateName) {
        super(plugin, player, "&6物品条件管理 - " + templateName, 54);
    ...
    private ItemStack createConditionDisplayItem(ItemConditionEntry entry, int index) {
        ...
        lore.add("&7条件编号: &e" + (index + 1));
        lore.add("&7物品类型: &a" + item.getType().name());
        lore.add("&7需要数量: &b" + entry.getAmount());
        
        if (item.hasItemMeta() && item.getItemMeta().hasDisplayName()) {
            lore.add("&7物品名称: &f" + item.getItemMeta().getDisplayName());
        }
        
        lore.add("");
        lore.add("&a左键点击编辑数量");
        lore.add("&c右键点击删除条件");
        lore.add("&eShift+左键复制物品");
    ...
    private void loadItemConditions() {
        ...
        try {
                    ...
                    if (materialName != null) {
                        try {
                            ...
                            if (displayName != null && !displayName.isEmpty()) {
                                item.editMeta(meta -> meta.setDisplayName(displayName));
                            }
            ...
            plugin.getLogger().warning("加载物品条件时出错: " + e.getMessage());
        }
    }
    ...
    private void handleSave() {
        try {
            ...
            plugin.getLogger().warning("保存物品条件配置时出错: " + e.getMessage());
        }
    }
