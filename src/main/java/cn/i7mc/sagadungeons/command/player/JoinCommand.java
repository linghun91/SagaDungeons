...
public class JoinCommand extends AbstractCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        // 检查副本状态是否为正在删除或已完成
        if (dungeon.getState() == cn.i7mc.sagadungeons.dungeon.DungeonState.DELETING ||
            dungeon.getState() == cn.i7mc.sagadungeons.dungeon.DungeonState.COMPLETED ||
            dungeon.getState() == cn.i7mc.sagadungeons.dungeon.DungeonState.TIMEOUT) {
            sendMessage(sender, "command.join.dungeon-not-available",
                    MessageUtil.createPlaceholders("id", idOrName));
            return;
        }
