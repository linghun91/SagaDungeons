...
public class SagaDungeons extends JavaPlugin {
    ...
    private void saveLanguageFiles() {
        ...
        // 保存中文GUI语言文件
        if (!new File(getDataFolder(), "lang_gui.yml").exists()) {
            saveResource("lang_gui.yml", false);
        }

        // 保存英文GUI语言文件
        if (!new File(getDataFolder(), "lang_gui_en.yml").exists()) {
            saveResource("lang_gui_en.yml", false);
        }
